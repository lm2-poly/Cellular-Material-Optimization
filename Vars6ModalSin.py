...
def DensityFunction(x0):
    ...
    y_min_bound = -85
    y_max_bound = 85
    l_x = x_max_bound - x_min_bound
    l_y = y_max_bound - y_min_bound

    sym = dict()
    # Ici, on vient créer les positions d'un cadran du problème
    s = 0
    for i in range(math.ceil(n_vars/2)):
        for j in range(math.ceil(n_vars/2)):
            if i >= j:
                sym[str(i) + " " + str(j)] = x0[s]
                s += 1

    #inc = 85/math.floor(n_vars/2)
    #for i in range(-math.floor(n_vars/2), math.floor(n_vars/2) + 1):
    #    temp = list()
    #    for j in range(-math.floor(n_vars/2), math.floor(n_vars/2) + 1):
    #        if abs(j) < abs(i):
    #            coords_dict = sym[str(abs(i)) + " " + str(abs(j))]
    #        else:
    #            coords_dict = sym[str(abs(j)) + " " + str(abs(i))]  # because of symmetry
    ...
    for i in x:
        for j in y:
            ...
            x_calcul[s], y_calcul[s], z[s] = modal_6_vars((i - 0.5) * 2 * half_panel, (j - 0.5) * 2 * half_panel, l_x, l_y, x0[0], x0[1], x0[2], x0[3], x0[4], x0[5])

            if (i - 0.5) ** 2 + (j - 0.5) ** 2 < (3 / 170) ** 2:  # radius
                z[s] = x0[0] + x0[1] + x0[2] + x0[3] + x0[4] + x0[5]
                # just in case, at initialisation, to avoid all black.
                if x0[0] + x0[1] + x0[2] + x0[3] + x0[4] + x0[5] == 0:
                    z[s] = 1
            s += 1
...
def modal_6_vars(x, y, l_x, l_y, a0, a1, a2, a3, a4, a5):
    f = a0 * (0.5 + 0.5 * math.cos(1 * math.pi * x / l_x) * math.cos(1 * math.pi * y / l_y))
    f += a1 * (0.5 + 0.5 * math.cos(3 * math.pi * x / l_x) * math.cos(3 * math.pi * y / l_y))
    f += a2 * (0.5 + 0.5 * math.cos(5 * math.pi * x / l_x) * math.cos(5 * math.pi * y / l_y))
    f += a3 * (0.5 + (0.25 * ((math.cos(1 * math.pi * x / l_x) * math.cos(3 * math.pi * y / l_y)) + (math.cos(3 * math.pi * x / l_x) * math.cos(1 * math.pi * y / l_y)))))
    f += a4 * (0.5 + (0.25 * ((math.cos(1 * math.pi * x / l_x) * math.cos(5 * math.pi * y / l_y)) + (math.cos(5 * math.pi * x / l_x) * math.cos(1 * math.pi * y / l_y)))))
    f += a5 * (0.5 + (0.25 * ((math.cos(3 * math.pi * x / l_x) * math.cos(5 * math.pi * y / l_y)) + (math.cos(5 * math.pi * x / l_x) * math.cos(3 * math.pi * y / l_y)))))
    f += -(a0 + a1 + a2 + a3 + a4 + a5)
    return x, y, f