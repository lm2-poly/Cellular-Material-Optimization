...
def initialization(n, D):
    ...
    samples = []
    np.random.seed(0)     # Here to change the seed
    while len(samples) < n:
...
if __name__ == '__main__':
    ...
    description = "Weighted Voronoi Stippler"
    ...
    pdf_filename = os.path.join("./data/PDF/", basename + "-stipple.pdf")
    png_filename = os.path.join("./data/WVS/", basename + "-stipple.png")
    dat_filename = os.path.join("./data/Voronois/", basename + "-stipple.npy")
    ...
    upload_file = "./data/Voronois/img_" + str(n_files-2).zfill(6) + "-stipple.npy"


#OD: UNCOMMENT 3 lines here
    #if True: # Test OD to change! LAST CHANGE UNTIL PUBLICATION      not os.path.exists(upload_file): # or args.force:   #) and n_files == 0:
    points = initialization(args.n_point, density)          # points = initialization(args.n_point, density)
        #print("Nb points:", args.n_point)
        #print("Nb iterations:", args.n_iter)
        #print('Nothing found')
    #else:
    #    points = np.load(upload_file)
