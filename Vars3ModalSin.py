...
def DensityFunction(x0):
    ...
    for i in x:
        for j in y:
            ...
            x_calcul[s], y_calcul[s], z[s] = modal_3_vars((i - 0.5) * 2 * half_panel, (j - 0.5) * 2 * half_panel, l_x, l_y, x0[0], x0[1], x0[2])
...
def modal_3_vars(x, y, l_x, l_y, a0, a1, a2):
    f = abs(a0 * math.cos(1 * math.pi * x / l_x) * math.cos(1 * math.pi * y / l_y))
    f += abs(a1 * math.cos(3 * math.pi * x / l_x) * math.cos(3 * math.pi * y / l_y))
    f += abs(a2 * math.cos(5 * math.pi * x / l_x) * math.cos(5 * math.pi * y / l_y))
    f += -(a0 + a1 + a2)
    return x, y, abs(f)